...
describe('Responsiveness Class', () => {
  ...
  it('should fetch data and calculate metric', async () => {
    ...
    const responsiveness = new Responsiveness('sharedProperty', repoOwner, repoName);
  ...
  it('should handle empty API responses in fetchData', async () => {
    ...
    expect(fetchDataResult).toBe('Fetched and processed data for Responsive Maintainer');
  });

  it('should handle API errors in fetchData', async () => {
    mocked(responsivenessApi.fetchIssueComments).mockRejectedValue(new Error('API Error'));
