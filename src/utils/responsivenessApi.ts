...
export const fetchIssueComments = async (owner: string, repo: string, issueNumber: number) => {
  const endpoint = `/repos/${owner}/${repo}/issues/${issueNumber}/comments`;
  return await getRequest(endpoint);
};

export const fetchPullRequestComments = async (owner: string, repo: string, pullNumber: number) => {
  const endpoint = `/repos/${owner}/${repo}/pulls/${pullNumber}/comments`;
  return await getRequest(endpoint);
};

export const fetchPullRequestMergeTime = async (owner: string, repo: string, pullNumber: number) => {
